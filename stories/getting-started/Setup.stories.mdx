...
<div class="docs-content">
  ...
  <div class="columns">
    <div class="column">
      ...
      <p>After setting it up, to run the storybook locally, go to the root folder (the one that contains the <code>.storybook</code> folder) and run <code>npm start</code>.</p>
  ...
  <div class="columns">
    <div class="column">
      <h2>Confetti</h2>
      <h3>Installing</h3>
      <p>Since currently all our codebase is inside the same repo, if you want to use our desing system by itself in outside projects, you'll need to ask Luciano on Slack to generate a package for you.</p>
      ...
      <p>To be able to use our styles, you'll have to have scss and svg support in your react project. If you do, just import our main scss file into yours by adding <code>@import '~labsystem/scss/main';</code> to your mais scss file.</p>
      <h3>Importing the fonts</h3>
      <p>Since we're using the design system internally and for testing purposes, if you want access to the fonts, talk with Debora or Luciano, so they can send you the HTML tag to import them. Our styles already reference the fonts, so after importing them, everything should be good to go.</p>
      ...
      <p>To where to import the components from, explore our <a href="https://github.com/labcodes/labstorybook/tree/master/labsystem/src">folder structure for components</a>. The compressed package's <code>dist</code> folder uses the same structure as the <code>src</code> folder from our source code, so if you, for example, want to import the Button component, you'll import it with <code>import &#123; Button &#125; from "labsystem/dist/Button/";</code> or <code>import Button from "labsystem/dist/Button/Button";</code>, because the source code for the Button component is inside the <code>labsystem/src/Button</code> folder.</p>
