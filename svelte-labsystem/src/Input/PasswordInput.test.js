...
describe("PasswordInput", () => {
  it("renders with base props and type=password", async () => {
    const { container } = render(
      html`<${PasswordInput} id="test" label="Some text"//>`
    );
  ...
  it("toggles password visibility when clicking the trailing icon", async () => {
    ...
    const trailingIcon = container.querySelectorAll(
      "span.lab-icon--eye-closed"
    );
    expect(trailingIcon).toHaveLength(1);
    expect(container.querySelectorAll("input[type='password']").length).toBe(1);

    await fireEvent.click(trailingIcon[0]);

    expect(container.querySelectorAll("input[type='text']")).toHaveLength(1);
    expect(
      container.querySelectorAll("span.lab-icon--eye-opened")
    ).toHaveLength(1);
  });

  it("sets state with isValid if it is passed by props", async () => {
    const { container } = render(
      html`<${PasswordInput} id="test" label="Some text" isValid=${false} //>`
    );

    expect(container).toMatchSnapshot();
    expect(container.querySelectorAll(".lab-input--invalid").length).toBe(1);
    expect(
      container.querySelector("input[type='password']").validity.valid
    ).toBeFalsy();
  });

  it("adds invalid css class when going from valid to invalid", async () => {
    const { container } = render(
      html`<${PasswordInput} id="test" label="Some text" required //>`
    );
  ...
  it("renders prefix", async () => {
    const { container } = render(
      html`<${PasswordInput} id="testInput" label="Test Input" prefix="R$" />`
    );
  ...
  it("renders suffix", async () => {
    const { container } = render(
      html`<${PasswordInput} id="testInput" label="Test Input" suffix=".com" />`
    );
  ...
  it("renders icon", async () => {
    const { container } = render(
      html`<${PasswordInput} id="testInput" label="Test Input" icon="star" />`
    );
  ...
  it("renders required icon", async () => {
    const { container } = render(
      html`<${PasswordInput} id="testInput" label="Test Input" required />`
    );
